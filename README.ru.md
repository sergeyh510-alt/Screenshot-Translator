...
Desktop-приложение на Python для захвата выбранной области экрана, распознавания текста с помощью vision-модели через OpenAI-совместимый API и сохранения результата в документ Word. Приложение ориентировано на сценарии, в которых текст на любом поддерживаемом языке находится внутри изображения, презентации, документа, удалённого рабочего стола или веб-интерфейса.
...
Точность зависит от качества изображения, языка и возможностей выбранной vision-модели. Стандартный prompt требует формат `SOURCE/TARGET`, но приложение не выполняет отдельную валидацию ответа модели.
